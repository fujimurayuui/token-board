...
- 各トークンのアンタップ/タップ欄に `-10` / `+10` ボタンを追加しました。
- iPhone Safariで連続タップしたときにズームが起きにくいよう、viewportとタッチ操作の設定を調整しました。
- ライフ・エネルギー機能は入れず、トークン管理専用の画面にしています。

GitHub Pages更新後に古い画面が残る場合は、URLの末尾に `?v=3` を付けて開き直してください。
